...
- [Development guidelines](#development-guidelines)
    - [General KEEPsCoin Core](#general-keepscoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play KPCs (tKPC)" on the test network, if you
are testing multi-machine code that needs to operate across the internet.

If you are testing something that can run on one machine, run with the `-regtest` option.
In regression test mode, blocks can be created on-demand;

### DEBUG_LOCKORDER

KEEPsCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/keepscoin-official/keepscoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_keepscoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_keepscoin --log_level=test_suite
$ valgrind -v --leak-check=full src/keepscoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_keepscoin.coverage/index.html`.
```

**Sanitizers**

KEEPsCoin Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/kpcoin/kpcoin/issues/12691)
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9111.
...
- BitcoinMiner : Generates kpcoins (if wallet is enabled).
...
However, in open source software such as KEEPsCoin Core, where everyone uses
...
pay attention to for reviewers of KEEPsCoin Core code.

General KEEPsCoin Core
--------------------------
...
- Avoid using locale dependent functions if possible.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: KEEPsCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `KPCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef KPCOIN_FOO_BAR_H
#define KPCOIN_FOO_BAR_H
...
#endif // KPCOIN_FOO_BAR_H
```
...
Some of these are maintained by active developers of KEEPsCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against KEEPsCoin Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/kpcoin-core/secp256k1/ ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/kpcoin-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors it will cause problems with KEEPsCoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to KEEPsCoin's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:keepscoin/keepscoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `keepscoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `keepscoin-cli` and the GUI debug console use this table to determine how to
