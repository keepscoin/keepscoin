...
*Setup instructions for a gitian build of KEEPsCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the KEEPsCoin
...
- [Building KEEPsCoin](#building-keepscoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-kpcoin-host/README.md).
...
Clone the git repositories for keepscoin and gitian and then checkout the keepscoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the keepscoin repository
...
To build KEEPsCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the keepscoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/keepscoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/keepscoin-official/keepscoin
...
For example:
```bash
URL=https://github.com/konez2k/keepscoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit keepscoin=${COMMIT} --url keepscoin=${URL} ../keepscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit keepscoin=${COMMIT} --url keepscoin=${URL} ../keepscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit keepscoin=${COMMIT} --url keepscoin=${URL} ../keepscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/keepscoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/keepscoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/keepscoin-build.assert
```
...
[keepscoin/gitian.sigs](https://github.com/keepscoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
