...
/**
 * Fees smaller than this (in ukpc) are considered zero fee (for transaction creation)
 * We are ~100 times smaller then kpcoin now (2015-06-23), set minTxFee 10 times higher
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    AvailableCoins(vPossibleCoins, true, nullptr, false, ONLY_10000);
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 KPC.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-kpc-address
                    CScript scriptChange;
