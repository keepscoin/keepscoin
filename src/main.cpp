...
#if defined(NDEBUG)
#error "KEEPsCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ukpc) are considered zero fee (for relaying and mining)
 * We are ~100 times smaller then kpcoin now (2015-06-23), set minRelayTxFee only 10 times higher
 * so it's still 10 times lower comparing to kpcoin.
 */
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < 100)
    {
        nSubsidy = 30000000 * COIN;			// 프리마이닝 갯수 = 30000000 * 100
    }
    else if (100 <= nHeight && nHeight < Params().LAST_POW_BLOCK())
    {
        nSubsidy = 0.1 * COIN;
    }
    else
        nSubsidy = 200 * COIN;				// pos 마이닝 갯수
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    ret = blockValue * 0.90; // 85% of block reward
...
{
    RenameThread("keepscoin-scriptch");
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  version=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f  cache=%u\n",
        chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), chainActive.Tip()->nVersion, log(chainActive.Tip()->nChainWork.getdouble()) / log(2.0),
        (unsigned long)chainActive.Tip()->nChainTx, DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
...
static bool ActivateBestChainStep(CValidationState& state, CBlockIndex* pindexMostWork, CBlock* pblock, bool fAlreadyChecked)
{
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        BOOST_REVERSE_FOREACH (CBlockIndex* pindexConnect, vpindexToConnect) {
            if (!ConnectTip(state, pindexConnect, pindexConnect == pindexMostWork ? pblock : nullptr, fAlreadyChecked)) {
                if (state.IsInvalid()) {
...
bool ActivateBestChain(CValidationState& state, CBlock* pblock, bool fAlreadyChecked)
{
    ...
    do {
        ...
        while (true) {
            ...
            if (!ActivateBestChainStep(state, pindexMostWork, pblock && pblock->GetHash() == pindexMostWork->GetBlockHash() ? pblock : nullptr, fAlreadyChecked))
                return false;
