...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars KPCOIND and KPCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

KPCOIND=${KPCOIND:-${1}/kpcoind}
CLI=${KPCOINCLI:-${1}/kpcoin-cli}
...
$KPCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$KPCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
