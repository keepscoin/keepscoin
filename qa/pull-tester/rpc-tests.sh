...
export KPCOINCLI=${BUILDDIR}/qa/pull-tester/run-kpcoin-cli
export KPCOIND=${REAL_KPCOIND}
...
if [ "x${ENABLE_KPCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and kpcoind must all be enabled"
fi
